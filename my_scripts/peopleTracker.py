...
def detect_and_track(video_reader, masa_model, masa_test_pipeline, texts="person", unified=True, test_pipeline=None, det_model=None, no_post=False, fp16=False):
    frame_idx = 0
    instances_list = []
    frames = []
    ...
    for frame in track_iter_progress((video_reader, len(video_reader))):
        if unified:
            ...
            # what i can do here is to check and draw the bboxes...how many are they? if more then 1 per each element or id i could use non-max suppression
            # but i think that the model is already doing this, so i can just use the bboxes as they are
            # 
            # prima di tutto devo verificare la dimensione delle features, in teoria devono sempre essere a numero fissato, ma non sono sicuro 
...
def detect_mode(args):
    ...
    instances_list, frames = detect_and_track(video_reader,
                                    masa_model,
                                    masa_test_pipeline,
                                    args.texts,
                                    args.unified,
